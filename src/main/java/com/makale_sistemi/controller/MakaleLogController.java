...
@RestController
@RequestMapping("/api/v1/crud/makale-log")
@RequiredArgsConstructor
public class MakaleLogController {
    private final Logger logger = LoggerFactory.getLogger(this.getClass());

    private final IMakaleLogService makaleLogService;

    @GetMapping(value = "/find-all")
    @Operation(summary = "List All", description = "Find All Records")
    public ResponseEntity<ServiceResponse> findAll() {
        try {
            List<MakaleLogDto> dtoList = makaleLogService.findAll();
    ...
    public ResponseEntity<ServiceResponse> findAllWithPageable(Pageable pageable) {
        try {
            Page<MakaleLogDto> dtoPage = makaleLogService.findAllWithPageable(pageable);
            return ResponseEntity.ok(ServiceResponse
                    .builder()
                    .success(true)
                    .statusCode(200)
    ...
    public ResponseEntity<ServiceResponse> findById(@PathVariable Long id) {
        try {
            MakaleLogDto dto = makaleLogService.findById(id, true);
            return ResponseEntity.ok(ServiceResponse
                    .builder()
                    .success(true)
                    .statusCode(200)
    ...
    public ResponseEntity<ServiceResponse> saveOrUpdate(@RequestBody MakaleLogDto dto) {
        try {
            MakaleLogDto responseDto = makaleLogService.saveOrUpdate(dto);
            return ResponseEntity.ok(ServiceResponse
                    .builder()
                    .success(true)
                    .statusCode(200)
    ...
    public ResponseEntity<ServiceResponse> deleteById(@PathVariable Long id) {
        try {
            makaleLogService.deleteById(id);
            return ResponseEntity.ok(ServiceResponse
                    .builder()
                    .success(true)
                    .statusCode(200)
