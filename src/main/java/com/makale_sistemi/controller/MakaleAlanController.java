...
@RestController
@RequestMapping("/api/v1/crud/makale-alan")
@RequiredArgsConstructor
public class MakaleAlanController {
    private final Logger logger = LoggerFactory.getLogger(this.getClass());

    private final IMakaleAlanService alanService;

    @GetMapping(value = "/find-all")
    @Operation(summary = "List All", description = "Find All Records")
    public ResponseEntity<ServiceResponse> findAll() {
        try {
            List<MakaleAlanDto> dtoList = alanService.findAll();
    ...
    public ResponseEntity<ServiceResponse> findAllWithPageable(Pageable pageable) {
        try {
            Page<MakaleAlanDto> dtoPage = alanService.findAllWithPageable(pageable);
            return ResponseEntity.ok(ServiceResponse
                    .builder()
                    .success(true)
                    .statusCode(200)
    ...
    public ResponseEntity<ServiceResponse> findById(@PathVariable Long id) {
        try {
            MakaleAlanDto dto = alanService.findById(id, true);
            return ResponseEntity.ok(ServiceResponse
                    .builder()
                    .success(true)
                    .statusCode(200)
    ...
    public ResponseEntity<ServiceResponse> saveOrUpdate(@RequestBody MakaleAlanDto dto) {
        try {
            MakaleAlanDto responseDto = alanService.saveOrUpdate(dto);
            return ResponseEntity.ok(ServiceResponse
                    .builder()
                    .success(true)
                    .statusCode(200)
