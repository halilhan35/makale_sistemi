package com.makale_sistemi.service.impl;

import com.makale_sistemi.dto.MakaleDto;
import com.makale_sistemi.entity.Makale;
import com.makale_sistemi.mapper.MakaleMapper;
import com.makale_sistemi.repository.MakaleRepository;
import com.makale_sistemi.service.IMakaleService;
import lombok.RequiredArgsConstructor;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.stereotype.Service;
import com.makale_sistemi.constant.ApplicationConstants;
import java.util.List;
import java.util.Optional;

@Service
@RequiredArgsConstructor
public class MakaleService implements IMakaleService {
    private final MakaleRepository repository;
    private final MakaleMapper mapper;

    public List<MakaleDto> findAll() {
        return mapper.entityListToDtoList(repository.findAll());
    }

    public Page<MakaleDto> findAllWithPageable(Pageable pageable) {
        Page<Makale> entityPage = repository.findAll(pageable);
        return entityPage.map(mapper::entityToDto);
    }

    public MakaleDto findById(Long id, boolean throwException) throws Exception {
        if (id == null) {
            if (throwException) {
                throw new Exception(String.format(ApplicationConstants.ZORUNLU_UYARI_MESAJI, ApplicationConstants.id));
            }

            return null;
        }

        Optional<Makale> entityOpt = repository.findById(id);

        if (entityOpt.isPresent()) {
            return mapper.entityToDto(entityOpt.get());
        }

        if (throwException) {
            throw new Exception(String.format(ApplicationConstants.ID_ILE_KAYIT_BULUNAMADI_UYARI_MESAJI, id));
        }

        return null;
    }

    public MakaleDto saveOrUpdate(MakaleDto dto) throws Exception {
        if (dto == null) {
            throw new Exception(String.format(ApplicationConstants.ZORUNLU_UYARI_MESAJI, ApplicationConstants.RequestBody));
        }

        if (dto.getId() != null) {
            findById(dto.getId(), true);
        }

        Makale entity = mapper.dtoToEntity(dto);
        repository.save(entity);
        return mapper.entityToDto(entity);
    }

    public void deleteById(Long id) throws Exception {
        MakaleDto dto = findById(id, true);
        repository.delete(mapper.dtoToEntity(dto));
    }
}
